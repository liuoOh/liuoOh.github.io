...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Liuooh")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Liuooh %c 你正在访问Liuoohの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Liuooh") ? title.substring(0, title.length - 9) : title;
    navigator.clipboard.writeText('Liuooh的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/assets/bg_clean.jpg);
    --darkmode-bg:url(/assets/bg_clean.jpg);
    --mobileday-bg: url(/assets/bg_clean.jpg);
    --mobilenight-bg: url(/assets/bg_clean.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
